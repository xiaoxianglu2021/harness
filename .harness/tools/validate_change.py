...
class Validator:
    ...
    def validate_index(self) -> list[IndexEntry]:
        ...
        for line in self.index_path.read_text(encoding="utf-8").splitlines():
            ...
            if status == "active" and not session:
                self.fail("index.active_unbound",
                          f"{change}: active change has no Session binding "
                          f"(see .harness/rules/parallel.md §3)")
            if session:
                if not SESSION_RE.match(session):
                    self.fail("index.session_invalid", f"{change}: bad session id `{session}`")
                elif session in active_sessions:
                    self.fail("index.session_reused",
                              f"session {session} bound to both "
                              f"{active_sessions[session]} and {change}")
                elif status == "active":
                    active_sessions[session] = change
        ...
        if sessions_file.exists():
            ...
            for entry in entries:
                if entry.status != "active":
                    continue
                row = self._index_session_of(entry.change)
                if row and row not in live:
                    self.warn("index.session_not_live",
                              f"{entry.change}: bound session {row} is not live; "
                              f"take over (bind) or abandon before integrate")
        return entries

    def _index_session_of(self, change: str) -> str | None:
        for line in self.index_path.read_text(encoding="utf-8").splitlines():
    ...
    def _live_sessions(sessions_file: Path) -> set[str]:
        ...
        for line in sessions_file.read_text(encoding="utf-8").splitlines():
            ...
            if len(cols) >= 4 and cols[0].startswith("sess-") and cols[3] == "live":
                live.add(cols[0])
        return live
    ...
    def validate_gate_records(self, change_dir: Path, text: str, fields: dict[str, str]) -> None:
        ...
        for record in records:
            ...
            if status == "done" and mechanical in {"fail", "blocked"} and human != "rejected":
