...
> 来源: [obra/superpowers](https://github.com/obra/superpowers) `skills/subagent-driven-development`（MIT License, © 2025 Jesse Vincent）。原文保持不变，仅追加本节。

When used inside Harness Engineering, this Skill is subordinate to `.harness/rules/flow-standard.md`（隔离协议与 Agent Dispatch Table）, `.harness/rules/gates.md`, `.harness/agents/*.md`, and `.harness/skills/README.md`. If this Skill conflicts with those files, the Harness files win.

在 Harness Standard-flow Phase 4/5：本 Skill 的 "fresh subagent per task" 语义即 Orchestrator 隔离协议；"ledger" 对应 change 产物归档；"Rulings" 决策记录写入 `summary.md`。本 Skill 不得改变 Phase 边界、Gate 判定与 Human Approval 流程；"Continuous execution / Rulings, not stalls" 仅适用于实现子步骤内部，不适用于 Gate fail/blocked（那必须 Stop-the-Line）。
