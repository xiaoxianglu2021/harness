...
## 2026-09-12 | Gate 证据相对路径以 change 目录为基准

- 问题: Phase 1 Gate 的 Artifact path 使用仓库相对路径，validator 报 gate.artifact_missing。
- 根因: validator 将非绝对路径相对于当前 change 目录解析；Orchestrator 写入了重复的 `.harness/changes/{id}` 前缀。
- 影响: Gate 证据文件实际存在但机械校验失败，不能请求阶段确认。
- 修复: 改用 `request_analysis/understanding.md`，保留原始失败证据并重新运行 validator。
- 预防: 写 Gate 时使用 change 相对路径；写完 Gate 后必须重新校验，不把仓库可点击路径直接复制为 Gate Artifact path。
