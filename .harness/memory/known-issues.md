...
## 2026-09-12 | Windows 的 python3 命中应用执行别名

- 描述: 当前 Windows Git Bash 中 `python3` 解析到 WindowsApps 占位入口，执行 validator 退出 49；`python` 为可运行的 Anaconda Python 3.10.9。系统级命令解析尚未修改。
- 影响范围: 本机直接执行文档中的 `python3` 工具命令；不代表 validator 校验逻辑失败。
- 临时方案: 每个 Git Bash 会话先确认 `python --version` 是 Python 3，再定义 `python3() { PYTHONUTF8=1 python -B -X utf8 "$@"; }`；独立工具 shell 需在同一命令中重新定义。此方式运行完整 validator 已返回 0。
- 计划修复: 在 `chore-windows-environment-20260912` 中补充 README 的项目级执行说明；不自动修改系统 PATH、应用别名或全局 shell 配置。若未来要求永久修复系统入口，另行获得用户授权。
